...
* `[$url=||example.com/content/*]##div.textad` hides a `div` with a class `textad` at addresses like `http://example.com/content/article.html` and even `http://subdomain.example.com/content/article.html`.
