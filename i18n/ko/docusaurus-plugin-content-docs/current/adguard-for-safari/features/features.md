---
title: 기능
sidebar_position: 1
---

- [일반](adguard-for-safari/features/general.md)

  Basic features, including notifications, update intervals, and logging level

- [필터](/archive/adguard-for-safari/features/filters.md)

  광고 차단을 맞춤 설정할 수 있는 다양한 필터

- [콘텐츠 차단기](/archive/adguard-for-safari/features/content-blockers/content-blockers.md)

  Apple의 제한 사항을 준수하는 6가지 주제별 콘텐츠 차단기

- [사용자 규칙](/archive/adguard-for-safari/features/rules.md)

  광고 차단을 세밀하게 조정하는 맞춤 필터링 규칙

- [정보](/archive/adguard-for-safari/features/about.md)

  개인정보 보호 문서, GitHub 리포지토리 및 버전 기록에 대한 링크가 있는 섹션
