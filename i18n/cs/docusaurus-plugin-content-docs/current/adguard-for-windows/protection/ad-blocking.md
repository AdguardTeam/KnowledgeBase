---
title: Blokování reklam
sidebar_position: 1
---

Sekce _Blokování reklam_ je zodpovědná za odstranění reklam z webových stránek, které navštěvujete, a z aplikací nainstalovaných ve vašem zařízení. K filtrování bannerů, vyskakovacích oken, slídičů a dalších rušivých prvků využívá AdGuard filtry — sady pravidel napsané ve [speciální syntaxi](/general/ad-filtering/create-own-filters). Pokud se chcete dozvědět více o tom, jak fungují, přečtěte si náš [článek o filtrech](/general/ad-filtering/how-ad-blocking-works).

![Ad blocking \*border](https://cdn.adtidy.org/content/kb/ad_blocker/windows/version_8/protection/ad_blocking.png)

Zde můžete:

- povolit základní ochranu,
- blokovat [reklamy ve vyhledávání a vlastní propagaci](/general/ad-filtering/search-ads),
- zakázat reklamy v nabídce Start ve Windows 11,
- povolit automatickou aktivaci jazykově specifických filtrů
- spravovat uživatelská pravidla.

Také můžete otevřít _Záznam filtrování_, abyste viděli všechny požadavky provedené prohlížeči a aplikacemi na vašem počítači. Zde můžete také zablokovat nebo odblokovat jakýkoli požadavek vytvořením vlastního pravidla. V _Editoru filtrů_ můžete vytvářet, přidávat a importovat vlastní pravidla a filtry.
