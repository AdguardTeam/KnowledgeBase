...
* Windows — işlem adını kullanın, örneğin `chrome.exe`. İşlem adının CoreLibs v1.12'den başlayarak büyük/küçük harfe duyarlı olmadığını unutmayın.
...
`$domain` limits the rule scope to requests made **from** the specified domains and their subdomains (as indicated by the [Referer](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Referer) HTTP header). Bir kurala birden fazla alan adı eklemek için ayırıcı olarak `|` karakterini kullanın.
...
> **Compatibility with different versions of AdGuard**
> 
> AdGuard for Windows, Mac, and Android often cannot accurately detect `navigator.sendBeacon()`. Güvenilir algılama için AdGuard Tarayıcı Uzantısını kullanın.
...
> **Compatibility with different versions of AdGuard**
> 
> AdGuard for Windows, Mac, Android often cannot accurately detect this type and sometimes detects it as [`$other`](#other-modifier) or [`$script`](#script-modifier). Güvenilir algılama için AdGuard Tarayıcı Uzantısını kullanın.
...
> **Compatibility with different versions of AdGuard**
> 
> * Gizlilik Modu, Windows, Mac, Android ve AdGuard Tarayıcı Uzantısı için AdGuard'da mevcuttur. All other products will ignore the rules with `$stealth` modifier.
