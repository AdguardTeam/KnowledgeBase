...
В [AdGuard для Chrome MV3][ext-mv3] правило с модификатором `$badfilter` применяется в DNR только в том случае, если оно полностью отменяет исходное правило. We cannot calculate it if it is only partially canceled. [Examples][badfilter-in-mv3].
...
The `:if-not()` pseudo-class is removed and is no longer supported. Правила с ним считаются недействительными.
