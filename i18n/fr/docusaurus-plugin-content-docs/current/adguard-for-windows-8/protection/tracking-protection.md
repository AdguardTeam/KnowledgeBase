...
Disables cached authorization data that can be used for tracking purposes. Peut interférer avec la fonctionnalité de certaines extensions de navigateur et sites web.
...
You can also enter a custom value in the _Custom Referer_ field. Pour utiliser le référent par défaut, laissez le champ vide.

Note that to be able to filter traffic, AdGuard applications intercept browser-to-server requests. Les requêtes adressées aux serveurs de publicité, de suivi et d'hameçonnage peuvent être modifiées avant d'être envoyées au serveur ou bloquées complètement.
...
You can enter any IP address you want to use in the corresponding field. Pour utiliser l'adresse IP par défaut, laissez le champ vide.
