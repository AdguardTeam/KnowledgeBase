...
- Simulace akce uživatele pomocí pravidla, které klikne na zadané tlačítko a přeruší jeho provádění 10 sekund po načtení. Jsou možné dvě možnosti:
    - Odmítnout (s výjimkou funkčních souborů cookies — v závislosti na systému CMP) — preferovaná možnost, protože je menší šance na načtení dalších analytických nástrojů
...
Tento filtr může obsahovat pravidla, která nejsou vhodná pro všechny uživatele. V takových případech se doporučuje tento filtr vypnout. O přidání pravidel do tohoto filtru rozhodují vývojáři filtru na základě jednotlivých pravidel.
