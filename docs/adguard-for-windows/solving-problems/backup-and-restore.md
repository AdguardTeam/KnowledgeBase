...
You don’t have to wait for an automatic backup — you can create a settings file at any time and import it later. In AdGuard for Windows, open *Settings* and go to the *App settings* tab. Click the three-dot menu in the top right corner.
...
1. Install AdGuard v8.0 again.

1. Exit AdGuard from the system tray to stop filtering.

1. Extract the contents of the backup ZIP file.

1. Replace the following files with the ones from the backup:
...
1. Launch AdGuard. Your previous settings, filters, and custom rules will be restored.
