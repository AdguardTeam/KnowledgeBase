...
Ujistěte se, že jsou Mac a druhé zařízení připojeny ke stejné síti, a zadejte port proxy serveru na zařízení, které chcete přesměrovat skrze proxy server (obvykle v nastavení sítě). To filter HTTPS traffic as well, [transfer AdGuard’s proxy certificate](https://local.adguard.org/cert) to this device. [Další informace o instalaci proxy certifikátu](/guides/proxy-certificate)
