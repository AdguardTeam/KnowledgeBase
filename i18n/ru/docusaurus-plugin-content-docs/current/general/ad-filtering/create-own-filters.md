...
A filter is a set of filtering rules applied to specific content, such as banners or popups. В AdGuard есть список базовых фильтров, созданных нашей командой. Они постоянно дорабатываются и дополняются, и, как мы надеемся, удовлетворяют требованиям большинства пользователей.
...
- `$specifichide` реализован путём сканирования существующих правил скрытия элементов и удаления целевого домена из их `if-domain`-массива.
    ...
    - Правила `$specifichide` охватывают только правила, таргетированные на тот же домен, что и само правило, поддомены игнорируются. Т.е. правило `@@||example.org^$specifichide` отключит `example.org##.banner`, но проигнорирует `sub.example.org##.banner`. This limitation may be lifted if [#72](https://github.com/AdguardTeam/SafariConverterLib/issues/72) is implemented.
