...
Sebbene AdGuard sia visualizzato come una VPN nelle impostazioni di sistema, non dovrebbe causare alcun conflitto, lavorando insieme ad altre app basate sulla VPN. Tuttavia, se si utilizza un'app basata su VPN scaricata al di fuori dell'App Store, esiste il rischio che utilizzi la vecchia API VPN che debba essere esclusa dal filtraggio:
...
Al momento, la modalità Estensione di Rete di AdGuard non è compatibile con [Little Snitch 5](https://obdev.at/products/littlesnitch/index.html). Quando entrambe sono in esecuzione, esiste il rischio di riscontrare problemi con il comportamento di varie app, anche se non sono filtrate da AdGuard. Questo problema è causato direttamente da un bug di Big Sur, e abbiamo già informato Apple a riguardo. Questo ci lascia credere che questo problema sarà risolto in uno dei prossimi aggiornamenti.

Tuttavia, disabilitare il monitoraggio della connessione in Little Snitch non risolve il problema, poiché non scarica l'estensione Little Snitch dalla memoria di sistema. Consigliamo di passare alla modalità di filtraggio [**Proxy Automatico**](#automatic-proxy), eseguendo AdGuard insieme a Little Snitch su Big Sur, almeno fino alla risoluzione del bug da parte di Apple.
...
Maggiori informazioni su come iniziare sono disponibili sul [sito web di Shadowsocks](https://shadowsocks.org/guide/what-is-shadowsocks.html).
