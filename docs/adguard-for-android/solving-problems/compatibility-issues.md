...
To be able to filter HTTPS traffic, AdGuard requires the user to add a certificate into the device's trusted user certificates. Unfortunately, UC-family browsers don't trust user certificates, so AdGuard cannot perform HTTPS filtering there.
