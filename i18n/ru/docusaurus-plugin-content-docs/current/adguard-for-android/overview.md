...
**Хотим ещё раз подчеркнуть, что Блокировщик AdGuard будет работать и без HTTPS-фильтрации. Однако он не сможет полноценно фильтровать рекламу на сайтах, использующих протокол HTTPS (а количество таких сайтов постоянно растёт). So if you want to use the full functionality of the app and get the best ad filtering quality and processing speed, we strongly recommend you to activate HTTPS filtering.**
...
AdGuard for Android is not an antivirus. Он не остановит загрузку вируса и не удалит уже существующие. Для полноценной защиты мы советуем использовать AdGuard в сочетании с антивирусом.
...
![Настройки приложений *mobile](https://cdn.adtidy.org/content/kb/ad_blocker/android/features/apps-management.png)
...
Прочтите [эту статью](/general/license/activation), чтобы узнать, как активировать лицензионный ключ.
