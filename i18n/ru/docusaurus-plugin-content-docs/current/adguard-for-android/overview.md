...
Основная цель любого блокировщика — блокировать рекламу, и у большинства программ AdGuard одинаковые алгоритмы фильтрации. The mechanism of filtering ads is described [here](/general/ad-filtering/how-ad-blocking-works).
...
AdGuard позволяет создавать собственные фильтры, используя те же типы правил, что и в наших фильтрах. If you want to try your hand at this, we would recommend studying [the article on creating your own ad filters](/general/ad-filtering/create-own-filters) which describes the AdGuard rules syntax in detail and provides plenty of examples.

> Совет: используйте функцию импорта, чтобы перенести свои правила на другое устройство.

## HTTPS-фильтрация

Сейчас большинство сайтов работают через HTTPS — защищённый веб-протокол. It happened so that most advertisers use this protocol as well, this is why [HTTPS filtering](/general/https-filtering/what-is-https-filtering) is really crucial for a modern ad blocker.
...
К счастью, модуль Антифишинг от AdGuard защитит вас от посещения фишинговых и вредоносных сайтов. Он предупредит вас при попытке загрузить вредоносную программу. In case you want to learn more about Browsing Security, read [this article](/general/browsing-security).
...
* Нажав на иконку батареи на главном экране приложения, вы попадёте на экран со статистикой использования батареи. System battery stats [are often wrong](../solving-problems/battery), so we decided to have our own, one that would reflect the real state of things
