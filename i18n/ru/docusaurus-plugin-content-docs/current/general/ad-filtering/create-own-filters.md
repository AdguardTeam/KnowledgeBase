...
* Windows — используйте имя процесса, например, `chrome.exe`. Note that the process name is case-insensitive starting with CoreLibs v1.12.
...
Модификатор `$all` состоит из модификаторов [`$document`](#document-modifier), [`$popup`](#popup-modifier) и [`$csp`](#csp-modifier). Например, правило `||example.org^$all` конвертируется в такой набор правил:
```
