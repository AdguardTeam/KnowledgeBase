---
title: Fonctionnalités
sidebar_position: 1
---

- [Fonctionnalités générales](/archive/adguard-for-safari/features/general.md)

  Basic features, including notifications, update intervals, and logging level

- [Filtres](/archive/adguard-for-safari/features/filters.md)

  Les filtres différents qui permettent de personnaliser le blocage des publicités

- [Bloqueur de contenu AdGuard](/archive/adguard-for-safari/features/content-blockers/content-blockers.md)

  Six bloqueurs de contenu thématiques qui respectent les restrictions d'Apple

- [Règles utilisateur](/archive/adguard-for-safari/features/rules.md)

  Règles de filtrage personnalisées pour affiner le blocage de publicité

- [À propos](/archive/adguard-for-safari/features/about.md)

  Une section avec des liens vers des fichiers de confidentialité, le dépôt GitHub et l'historique des versions
