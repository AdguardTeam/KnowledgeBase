...
![Filtrado \*mobile_border](https://cdn.adtidy.org/content/kb/ad_blocker/android/features/filtering.png)
...
Las extensiones te permiten añadir scripts de usuario y userstyles.
...
Los estilos de usuario son como los scripts de usuario: permiten cambiar el aspecto de las páginas web en el navegador sin modificar tu contenido. Funcionan añadiendo estilos CSS a los estilos existentes del sitio web. Por ejemplo, si quieres un tema oscuro en un sitio web que no ofrece uno, puedes usar un estilo de usuario para conseguirlo.

Para añadir un estilo de usuario, ve a _Configuración_ → _Filtrado_ → _Extensiones_ → _Añadir extensión_ → _Importar desde un archivo o una URL_. Puedes encontrar estilos de usuario prediseñados [en línea](https://userstyles.world/).

Para crear tu propio estilo, toca _Añadir extensión_ → _Crear estilo de usuario_.
...
Ten en cuenta que el filtrado de DNS seguirá activo en el modo de enrutamiento _Proxy automático_, y es posible que tu dispositivo no pueda filtrar las solicitudes de DNS de aplicaciones específicas. En el modo de enrutamiento _VPN_, las aplicaciones se excluyen de la VPN local mediante mecanismos del sistema. Por lo tanto, el solucionador del sistema detecta qué aplicaciones están incluidas en la VPN y cuáles no. Les permite acceder a la VPN local o eludirla. Sin embargo, las solicitudes de DNS provienen del solucionador del sistema, no de la aplicación. En el modo _Proxy automático_, solo es posible excluir el tráfico de las aplicaciones, no sus consultas DNS, porque AdGuard las considera consultas del sistema.
...
- Regístrate para activar tu período de prueba de 14 días. El código de activación se puede utilizar en diferentes dispositivos y plataformas
