...
You can save your settings configuration by clicking the _Export settings_ button. Настройки будут сохранены в виде файла .json. To upload the previously saved settings configuration, use the _Import settings_ function. Вы можете использовать эту функцию, чтобы быстро переключаться между различными профилями настроек или перенести конфигурацию настроек в другой браузер.
