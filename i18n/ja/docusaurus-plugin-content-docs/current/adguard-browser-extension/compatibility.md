...
拡張機能とスタンドアロンアプリの両方をPCにインストールする場合、互換性の問題が発生する可能性があります。 問題は、場合によってはアプリケーションと拡張機能が干渉しあう可能性があることです。 一部のウェブサイトはデスクトップアプリでは保護されなくなり、オプションの少ない拡張機能でのみ保護されるようになってしまいます。 また、ウェブサイトによってはフィルタリングが正しく機能しない場合があります。 Therefore, we don’t recommend using the extension together with the application.
